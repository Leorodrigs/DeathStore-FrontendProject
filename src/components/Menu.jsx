...
export const Menu = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 h-full w-full md:w-80 bg-black shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${
            ...
            {user?.isAdmin && (
              <>
                ...
                <Link
                  to="/admin/clients"
                  onClick={onClose}
                  className="flex items-center space-x-3 w-full px-3 py-2 text-white hover:bg-gray-800 rounded-lg transition-all duration-200 group"
                >
                  <svg
                    className="w-5 h-5 text-gray-400"
                    fill="none"
